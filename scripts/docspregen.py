...
.. list-table::
    ...
    * - :ref:`framework_{name}`
    ...
    assert known >= set(
        frameworks
    ), f"Unknown frameworks {set(frameworks) - known} "
    return lines
...
def generate_platform(name, rst_dir):
    ...
    lines.extend(
        (
            ...
            "",
            ":Configuration:",
            f"  :ref:`projectconf_env_platform` = ``{p.name}``",
            "",
            p.description,
        )
    )
            ...
            lines.append(
                (
                    ...
                    % (
                        ...
                        manifest['title'],
                        campaign_url(
                            f"{p.repository_url[:-4]}/tree/master/examples"
                        ),
                    )
                )
            )
    ...
    for manifest in PLATFORM_MANIFESTS:
        ...
        if isdir(platform_examples_dir):
            for item in os.listdir(platform_examples_dir):
                ...
                if not isdir(example_dir) or not os.listdir(example_dir):
                    continue
                url = f"{github_url}/tree/master/examples/{item}"
                examples_md_lines.append(f"* [{item}]({url})")
        ...
        for framework in API_FRAMEWORKS:
            ...
            lines = [f"- [{p.title}]({github_url})"]
            lines.extend(f"  {l}" for l in examples_md_lines)
            lines.append("")
            framework_examples_md_lines[framework['name']].extend(lines)

        # Root README
        line = f"* [{p.title}]({github_url}/tree/master/examples)"
        if p.is_embedded():
            ...
            fp.write(
                framework_readme_tpl.format(
                    ...
                    examples="\n".join(
                        framework_examples_md_lines[framework['name']])))
        url = campaign_url(
            f"https://docs.platformio.org/en/latest/frameworks/{framework['name']}.html#examples",
            source="github",
            medium="examples",
        )
        frameworks.append(f"* [{framework['title']}]({url})")
