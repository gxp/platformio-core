...
class LibBuilderBase(object):
    ...
    def __contains__(self, path):
        ...
        if WINDOWS:
            p1 = p1.lower()
            p2 = p2.lower()
        return True if p1 == p2 else commonprefix((p1 + sep, p2)) == p1 + sep
    ...
    def build_dir(self):
        ...
        return join("$BUILD_DIR", f"lib{lib_hash}", basename(self.path))
...
class ArduinoLibBuilder(LibBuilderBase):
    ...
    def src_filter(self):
        ...
        for ext in piotool.SRC_BUILD_EXT + piotool.SRC_HEADER_EXT:
            ...
            if is_utility:
                src_filter.append(f"+<utility{sep}*.{ext}>")
        return src_filter
    ...
    def is_platforms_compatible(self, platforms):
        ...
        return (
            util.items_in_list(platforms, items)
            if items
            else LibBuilderBase.is_platforms_compatible(self, platforms)
        )
...
class MbedLibBuilder(LibBuilderBase):
    ...
    def load_manifest(self):
        return (
            fs.load_json(join(self.path, "module.json"))
            if isfile(join(self.path, "module.json"))
            else {}
        )
    ...
    def get_include_dirs(self):
        ...
        if not self._manifest:
            for root, _, __ in os.walk(self.path):
                ...
                if any(s in part for s in (f"{sep}.", "test", "example")):
                    continue
...
class PlatformIOLibBuilder(LibBuilderBase):
    ...
    def is_frameworks_compatible(self, frameworks):
        ...
        return (
            util.items_in_list(frameworks, items)
            if items
            else LibBuilderBase.is_frameworks_compatible(self, frameworks)
        )
    ...
    def _print_deps_tree(root, level=0):
        ...
        for lb in root.depbuilders:
            ...
            if vcs_info and vcs_info.get("version"):
                title += f' #{vcs_info.get("version")}'
            sys.stdout.write(f"{margin}|-- {title}")
            if int(ARGUMENTS.get("PIOVERBOSE", 0)):
                if vcs_info:
                    sys.stdout.write(f' [{vcs_info.get("url")}]')
                sys.stdout.write(" (")
